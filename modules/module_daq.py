...
class DAQ():
    ...
    def __init__(self, frequency=100000, duty_cycle=0.5):
        ...
        try:
            ...
            DAQmxCreateCOPulseChanFreq(self.counterHandle, 'Dev1/ctr0', '', DAQmx_Val_Hz, DAQmx_Val_Low, 0.0, self.frequency, self.duty_cycle)
            
            self.doHandle = TaskHandle(0)
            DAQmxCreateTask("", byref(self.doHandle))
            DAQmxCreateDOChan(self.doHandle, 'Dev1/port0/line0:4', '', DAQmx_Val_ChanPerLine) 
    ...
    def set_xyz(self, pos_x, pos_y, pos_z):
        try:
            DAQmxWriteAnalogF64(self.piezoHandle, 1, True, 10.0, DAQmx_Val_GroupByChannel, np.array([pos_x, pos_y], dtype=np.float64), None, None)
    ...
    def set_galvo(self, axis_x, axis_y):
        try:
            DAQmxWriteAnalogF64(self.galvoHandle, 1, True, 10.0, DAQmx_Val_GroupByChannel, np.array([axis_x, axis_y], dtype=np.float64), None, None)
    ...
    def scan_galvo(self, axis_x, axis_y):
        ...
        try:
            DAQmxCfgSampClkTiming(self.galvoHandle, r'100kHzTimebase', self.frequency, DAQmx_Val_Rising, DAQmx_Val_FiniteSamps, samples)
            DAQmxWriteAnalogF64(self.galvoHandle, samples, False, -1, DAQmx_Val_GroupByChannel, np.array([axis_x, axis_y], dtype=np.float64), None, None)
            DAQmxStartTask(self.galvoHandle)
        except:
            errBuff = create_string_buffer(b"", 2048)
            PyDAQmx.DAQmxGetExtendedErrorInfo(errBuff, 2048)
    ...
    def Trig_open(self, do_sequences):
        ...
        try:
            DAQmxCfgSampClkTiming(self.doHandle, r'100kHzTimebase', self.frequency, DAQmx_Val_Rising, DAQmx_Val_ContSamps, do_samples)
            DAQmxWriteDigitalLines(self.doHandle, do_samples, False, -1, DAQmx_Val_GroupByChannel, do_sequences.astype(np.uint8), None, None)
    ...
    def Trig_open_ao(self, do_sequences):
        do_channels, do_samples = do_sequences.shape
        try:
            DAQmxCfgSampClkTiming(self.doHandle, r'100kHzTimebase', self.frequency, DAQmx_Val_Rising, DAQmx_Val_FiniteSamps, do_samples)
            DAQmxWriteDigitalLines(self.doHandle, do_samples, False, -1, DAQmx_Val_GroupByChannel, do_sequences.astype(np.uint8), None, None)
            print("Data Written on DAQ\n")
        except:
            errBuff = create_string_buffer(b"", 2048)
    ...
    def Trigger_sequence(self, ao_sequences, do_sequences):
        try:
            ...
            DAQmxCfgSampClkTiming(self.piezoHandle, r'Ctr0InternalOutput', self.frequency, DAQmx_Val_Rising, DAQmx_Val_FiniteSamps, ao_samples)
            DAQmxCfgSampClkTiming(self.doHandle, r'Ctr0InternalOutput', self.frequency, DAQmx_Val_Rising, DAQmx_Val_FiniteSamps, do_samples)
            DAQmxWriteAnalogF64(self.piezoHandle, ao_samples, False, -1, DAQmx_Val_GroupByChannel, ao_sequences.astype(np.float64), None, None)
            DAQmxWriteDigitalLines(self.doHandle, do_samples, False, -1, DAQmx_Val_GroupByChannel, do_sequences.astype(np.uint8), None, None)
            print("Data Written on DAQ\n")
        except:
            errBuff = create_string_buffer(b"", 2048)
