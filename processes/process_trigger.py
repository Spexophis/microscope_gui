...
class Trigger_Sequence():
    ...
    def updata_parameters(self, sequence_time, sample_rate, axis_lengths, step_sizes, axis_start_pos, return_time, convFactors, analog_start, ttl_starts, ttl_ends):
    ...
    def generate_trigger_sequence_2d(self):
        ...
        temp = np.append(temp,  self.bp_decrease(np.linspace(0, 1, int(cycle_samples-startSamp)+return_samples)) * fast_axis_step_size * (fast_axis_positions-1))
        # temp = np.append(temp, np.linspace(1, 0, int(cycle_samples-startSamp)+return_samples) * fast_axis_step_size * (fast_axis_positions-1))
        ...
        cycle = np.ones((cycle_samples * fast_axis_positions + return_samples)) * middle_axis_step_size * (middle_axis_positions-1)
    ...
    def generate_trigger_sequence_3d(self):
        ...
        return_samples = self.return_time * self.sample_rate
        return_samples = int(np.ceil(return_samples))
        [fast_axis_size, middle_axis_size, slow_axis_size] = [(self.axis_lengths[i] / self.convFactors[i]) for i in range(3)]
        [fast_axis_step_size, middle_axis_step_size, slow_axis_step_size] = [(self.step_sizes[i] / self.convFactors[i]) for i in range(3)]
        [fast_axis_start, middle_axis_start, slow_axis_start] = [(self.axis_start_pos[i] / self.convFactors[i]) for i in range(3)]
        fast_axis_positions = 1 + int(np.ceil(fast_axis_size / fast_axis_step_size))
        middle_axis_positions = 1 + int(np.ceil(middle_axis_size / middle_axis_step_size))
        ...
        for i, start in enumerate(self.ttl_starts):
            ...
            digital_trigger_sequences.append(np.tile(temp, fast_axis_positions))
            digital_trigger_sequences[i] = np.append(digital_trigger_sequences[i], np.zeros(return_samples))
            digital_trigger_sequences[i] = np.tile(digital_trigger_sequences[i], middle_axis_positions * slow_axis_positions)
        ...
        cycle = np.ones(((cycle_samples * fast_axis_positions + return_samples) * middle_axis_positions)) * slow_axis_step_size * (slow_axis_positions-1)
        temp = np.append(temp, cycle)
        analog_trigger_sequences.append(temp)
        
        return np.asarray(analog_trigger_sequences), np.asarray(digital_trigger_sequences), positions
    
    def generate_trigger_sequence_beadscan_2d(self, l):
        digital_trigger_sequences = []
        analog_trigger_sequences = []
        cycle_samples = self.sequence_time * self.sample_rate
        cycle_samples = int(np.ceil(cycle_samples))
