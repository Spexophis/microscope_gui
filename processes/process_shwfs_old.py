...
control_matrix_zonal = tf.imread(r'C:\Users\ruizhe.lin\Documents\data\dm_files\20230412_132823_control_matrix.tif')
...
class WavefrontSensing:
    ...
    def __init__(self):
        ...
        self.x_center_base = 1037
        self.y_center_base = 1081
        self.x_center_offset = 1037
        self.y_center_offset = 1081
        self.px_spacing = 62  # spacing between each lenslet
    ...
    def _generate_influence_matrix(self, data_folder, method='zonal'):
        ...
        _msk = 1  # self._circular_mask(self.diameter / 2, self.diameter)
    ...
    def _get_correction(self, measurement, method='zonal'):
        ...
        if method == 'zonal':
            self._correction.append(list(np.matmul(control_matrix_zonal, _measurement)))
        elif method == 'modal':
            a = self._zernike_coefficients(_measurement, self.zslopes)
            self._correction.append(list(np.matmul(control_matrix_modal, a)))
        else:
            raise ValueError("Invalid method")
