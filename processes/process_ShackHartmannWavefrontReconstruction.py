...
# gradx_file = r'C:/Users/Public/Documents/python_code/sim_sona_p38/gradx.tif'
# grady_file = r'C:/Users/Public/Documents/python_code/sim_sona_p38/grady.tif'
...
class Wavefront_Reconstruction():
    ...
    def __init__(self):
        ...
        self.base = N.zeros((512, 512))
        self.offset = N.zeros((512, 512))
        self.wf = N.zeros((self.diameter, self.diameter))
    ...
    def GetAberration2img(self, baseimg, offsetimg, method=0):
        # initialize Arrays
        ...
        if (method == 0):
            ...
            self.gradx, self.grady = self.GetGradientsCorr(baseimg, offsetimg)
        ...
        if (method == 1):
            ...
            self.gradxy[0] = self.gradx
            self.gradxy[1] = self.grady
        gradx = N.pad(self.gradx, ((1, 1), (1, 1)), 'constant')
        grady = N.pad(self.grady, ((1, 1), (1, 1)), 'constant')
        self.extx, self.exty = self.hudgins_extend_mask0(gradx, grady)
        ...
        self.msk = self.circle(self.diameter / 2., self.diameter + 2)
        self.phicorr = self.phicorr * self.msk
        self.phicorr = self.phicorr[1:1 + self.diameter, 1:1 + self.diameter]
        # print('Wavefront Reconstruction Done!')
    ...
    def RemoveGlobalWaffle(self, phi):
        wmode = N.zeros((self.diameter + 2, self.diameter + 2))
        constant_num = 0
        constant_den = 0
    ...
    # def sharp(self,img):
    #     m = N.mean(img)
    #     # g = cv2.Laplacian(img, cv2.CV_32F)
    #     g = laplace(img)
    #     s = N.sum(N.abs(g)/m)
    #     return s

    # def setresponse(self):
    #     gradx = tf.imread(gradx_file)
    #     grady = tf.imread(grady_file)
    #     nz, nx, ny = gradx.shape
    #     gdx = gradx.reshape(nz, nx*ny)
    #     gdy = grady.reshape(nz, nx*ny)
    #     gradxy = N.concatenate((gdx,gdy),axis=1)
    #     r = N.linalg.pinv(gradxy)
    #     return r

    # def zonalcontrol(self,response,gradx,grady):
    #     nx, ny = gradx.shape
    #     msk = c.circle(self.diameter/2, self.diameter)
    #     gx = (gradx*msk).reshape(nx*ny)
    #     gy = (grady*msk).reshape(nx*ny)
    #     gxy = N.concatenate((gx,gy),axis=0)
    #     a = N.matmul(gxy,response)
    #     return a

    # def wf_recon_zonal(self, acts, infl, wfx, wfy):
    #     nz, nx, ny = infl.shape
    #     wf = N.zeros((wfx, wfy))
    #     for i in range(nz):
    #         wf = wf + infl[i,:,:]
